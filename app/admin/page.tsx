...
import { LogIn, LogOut, UserIcon, Plus, Save, Trash2, Edit, AlertCircle, X } from "lucide-react"
...
interface AboutData {
  ...
  selectedExhibitions: { title: string; venue: string; year: string; type: string }[]
}
...
export default function AdminPage() {
  ...
  const [aboutData, setAboutData] = useState<AboutData>({
    ...
    selectedExhibitions: [{ title: "", venue: "", year: "", type: "Group Show" }],
  })
        ...
        <Tabs defaultValue="artworks">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <ImageUpload
                        ...
                        onChange={(url) => setArtworkForm((prev) => ({ ...prev, image: url }))}
                        required
                      />
                    </div>
                  </div>
              ...
              <CardContent>
                <form onSubmit={handleExhibitionSubmit} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label>Exhibition Images</Label>
                    <ImageUpload
                      id="exhibition-image-upload"
                      value=""
                      onChange={(url) => {
                        if (url) setExhibitionForm((prev) => ({ ...prev, images: [...prev.images, url] }))
                      }}
                    />
                    {exhibitionForm.images.length > 0 && (
                      <div className="grid grid-cols-3 gap-2 mt-2">
                        {exhibitionForm.images.map((img, i) => (
                          <div key={i} className="relative group rounded overflow-hidden border aspect-video bg-muted">
                            <img src={img} alt="" className="w-full h-full object-cover" />
                            <button
                              type="button"
                              onClick={() => setExhibitionForm((prev) => ({
                                ...prev,
                    ...
                    <form onSubmit={handleProcessStepSubmit} className="space-y-4">
                      ...
                      <div>
                        <ImageUpload
                          ...
                          onChange={(url) => setProcessStepForm((prev) => ({ ...prev, image: url }))}
                          required
                        />
                      </div>
                    ...
                    <form onSubmit={handleStudioImageSubmit} className="space-y-4">
                      ...
                      <div>
                        <ImageUpload
                          ...
                          onChange={(url) => setStudioImageForm((prev) => ({ ...prev, image: url }))}
                          required
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <ImageUpload
                            ...
                            onChange={(url) => setVideoForm((prev) => ({ ...prev, thumbnail: url }))}
                            required
                          />
                        </div>
                        <div>
          ...
          <TabsContent value="about" className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <Button onClick={handleAboutSave} disabled={aboutSaving || aboutLoading}>
                <Save className="w-4 h-4 mr-2" />
                {aboutLoading ? "Loading..." : aboutSaving ? "Saving..." : aboutSaved ? "Saved!" : "Save Changes"}
              </Button>
            </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <ImageUpload
                  ...
                  onChange={(url) => setAboutData((p) => ({ ...p, profileImage: url }))}
                />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label>Photo Caption</Label>
                    <Input value={aboutData.photoCaption} onChange={(e) => setAboutData((p) => ({ ...p, photoCaption: e.target.value }))} placeholder="e.g. Studio Portrait" />
                  </div>
                  <div>
                    <Label>Photo Subcaption</Label>
                    <Input value={aboutData.photoSubcaption} onChange={(e) => setAboutData((p) => ({ ...p, photoSubcaption: e.target.value }))} placeholder="e.g. Lagos, 2023" />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label>Name</Label>
                    <Input value={aboutData.name} onChange={(e) => setAboutData((p) => ({ ...p, name: e.target.value }))} />
                  </div>
                  <div>
                    <Label>Location</Label>
                    <Input value={aboutData.location} onChange={(e) => setAboutData((p) => ({ ...p, location: e.target.value }))} placeholder="e.g. Lagos, Nigeria" />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label>Active Since (year)</Label>
                    <Input value={aboutData.activeSince} onChange={(e) => setAboutData((p) => ({ ...p, activeSince: e.target.value }))} placeholder="e.g. 2016" />
                  </div>
                  <div>
                    <Label>Email</Label>
                    <Input type="email" value={aboutData.email} onChange={(e) => setAboutData((p) => ({ ...p, email: e.target.value }))} />
                  </div>
                </div>
                <div>
                  <Label>Singulart Profile URL</Label>
                  <Input value={aboutData.singulartUrl} onChange={(e) => setAboutData((p) => ({ ...p, singulartUrl: e.target.value }))} placeholder="https://www.singulart.com/en/artist/..." />
                </div>
                <div>
                  <Label>Bio (intro paragraph)</Label>
                  <Textarea rows={4} value={aboutData.bio} onChange={(e) => setAboutData((p) => ({ ...p, bio: e.target.value }))} placeholder="Short biography shown at the top of the About page" />
                </div>
              </CardContent>
            </Card>
            ...
            {/* Artist Statement */}
            <Card>
              <CardHeader><CardTitle>Artist Statement</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {aboutData.statementParagraphs.map((para, i) => (
                  <div key={i}>
                    <div className="flex justify-between items-center mb-1">
                      <Label>Paragraph {i + 1}</Label>
                      {aboutData.statementParagraphs.length > 1 && (
                        <Button type="button" variant="ghost" size="sm" className="text-destructive h-7 px-2"
                          onClick={() => setAboutData((p) => ({ ...p, statementParagraphs: p.statementParagraphs.filter((_, idx) => idx !== i) }))}>
                          <Trash2 className="w-3 h-3" />
                        </Button>
                      )}
                    </div>
                    <Textarea rows={4} value={para}
                      onChange={(e) => setAboutData((p) => ({ ...p, statementParagraphs: p.statementParagraphs.map((s, idx) => idx === i ? e.target.value : s) }))} />
                  </div>
                ))}
                <Button type="button" variant="outline" size="sm"
                  onClick={() => setAboutData((p) => ({ ...p, statementParagraphs: [...p.statementParagraphs, ""] }))}>
            ...
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  ...
                  <Button type="button" variant="outline" size="sm"
                    onClick={() => setAboutData((p) => ({ ...p, education: [...p.education, { title: "", institution: "", year: "" }] }))}>
                  ...
                  <div key={i} className="p-4 border rounded-lg space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      {aboutData.education.length > 1 && (
                        <Button type="button" variant="ghost" size="sm" className="text-destructive h-7 px-2"
                          onClick={() => setAboutData((p) => ({ ...p, education: p.education.filter((_, idx) => idx !== i) }))}>
                          <Trash2 className="w-3 h-3" />
                        </Button>
                      )}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className="md:col-span-1">
                        <Label>Year</Label>
                        <Input value={edu.year} placeholder="2019"
                          onChange={(e) => setAboutData((p) => ({ ...p, education: p.education.map((d, idx) => idx === i ? { ...d, year: e.target.value } : d) }))} />
                      </div>
                      <div className="md:col-span-2">
                        <Label>Title / Degree</Label>
                        <Input value={edu.title} placeholder="e.g. BTech Marine Science"
                          onChange={(e) => setAboutData((p) => ({ ...p, education: p.education.map((d, idx) => idx === i ? { ...d, title: e.target.value } : d) }))} />
                      </div>
                    </div>
                    <div>
                      <Label>Institution</Label>
                      <Input value={edu.institution} placeholder="e.g. Federal University of Technology Akure"
                        onChange={(e) => setAboutData((p) => ({ ...p, education: p.education.map((d, idx) => idx === i ? { ...d, institution: e.target.value } : d) }))} />
                    </div>
                  </div>
                ))}
              </CardContent>
            ...
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <CardTitle>Selected Exhibitions (About page)</CardTitle>
                  <Button type="button" variant="outline" size="sm"
                    onClick={() => setAboutData((p) => ({ ...p, selectedExhibitions: [...p.selectedExhibitions, { title: "", venue: "", year: "", type: "Group Show" }] }))}>
                    <Plus className="w-4 h-4 mr-1" /> Add Exhibition
                  </Button>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                {aboutData.selectedExhibitions.map((ex, i) => (
                  <div key={i} className="p-4 border rounded-lg space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-muted-foreground">Exhibition {i + 1}</span>
                      {aboutData.selectedExhibitions.length > 1 && (
                        <Button type="button" variant="ghost" size="sm" className="text-destructive h-7 px-2"
                          onClick={() => setAboutData((p) => ({ ...p, selectedExhibitions: p.selectedExhibitions.filter((_, idx) => idx !== i) }))}>
                          <Trash2 className="w-3 h-3" />
                        </Button>
                      )}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        <Label>Title</Label>
                        <Input value={ex.title} placeholder="Exhibition title"
                          onChange={(e) => setAboutData((p) => ({ ...p, selectedExhibitions: p.selectedExhibitions.map((x, idx) => idx === i ? { ...x, title: e.target.value } : x) }))} />
                      </div>
                      <div>
                        <Label>Year</Label>
                        <Input value={ex.year} placeholder="2023"
                          onChange={(e) => setAboutData((p) => ({ ...p, selectedExhibitions: p.selectedExhibitions.map((x, idx) => idx === i ? { ...x, year: e.target.value } : x) }))} />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        <Label>Venue</Label>
                        <Input value={ex.venue} placeholder="Gallery or institution name"
                          onChange={(e) => setAboutData((p) => ({ ...p, selectedExhibitions: p.selectedExhibitions.map((x, idx) => idx === i ? { ...x, venue: e.target.value } : x) }))} />
                      </div>
                      <div>
                        <Label>Type</Label>
                        <Input value={ex.type} placeholder="e.g. Group Show, Solo"
                          onChange={(e) => setAboutData((p) => ({ ...p, selectedExhibitions: p.selectedExhibitions.map((x, idx) => idx === i ? { ...x, type: e.target.value } : x) }))} />
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="flex justify-end">
              <Button onClick={handleAboutSave} disabled={aboutSaving || aboutLoading} size="lg">
                <Save className="w-4 h-4 mr-2" />
                {aboutLoading ? "Loading..." : aboutSaving ? "Saving..." : aboutSaved ? "Saved!" : "Save All Changes"}
              </Button>
            </div>
          </TabsContent>
