...
export async function GET() {
  try {
    ...
    // Single aggregation: join artworks, compute artworkCount + coverImage + preview images
    const series = await db
      ...
      .aggregate([
        ...
        {
          $addFields: {
            artworkCount: { $size: "$_artworks" },
            coverImage: {
              $ifNull: ["$coverImage", { $arrayElemAt: ["$_artworks.image", 0] }],
            },
            images: { $slice: ["$_artworks.image", 3] },
          },
        },
        { $project: { _artworks: 0 } },
      ])
